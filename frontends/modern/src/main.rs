...
fn main() -> ExitCode {
    ...
    let tokens = match modernfront::lex(&source) {
        Ok(tokens) => tokens,
    ...
    let module = match modernfront::parse(tokens) {
        Ok(module) => module,
        Err(error) => return report(&input, error),
    };
    if syntax_only {
    ...
    let module_name = Path::new(&input)
        ...
        .unwrap_or("module");
    match modernfront::semantic::compile(&module, module_name) {
        Ok(hir) => {
            print!("{hir}");
            ExitCode::SUCCESS
        }
...
fn report(path: &str, error: modernfront::Diagnostic) -> ExitCode {
    eprintln!(
        "{path}:{}:{}: {}",
