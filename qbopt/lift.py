...
PAIRS = {0x23:(0x23,0x23,'and'), 0x0B:(0x0B,0x0B,'or'), 0x33:(0x33,0x33,'xor'),
         0x03:(0x13,0x03,'add'), 0x2B:(0x1B,0x2B,'sub')}
...
class Value:
    def __init__(s, op, alu=None, s1=None, s2=None, mem=None, at=None,
                 end=None, pair=0, src_pair=0, base=0x06, dlen=2):
        s.op, s.alu, s.s1, s.s2 = op, alu, s1, s2
        s.mem, s.at, s.end, s.pair = mem, at, end, pair
        s.src_pair, s.base, s.dlen = src_pair, base, dlen
    def __repr__(s):
        if s.op == LOAD:  return f"load [{s.mem:#06x}]"
        if s.op == ALUM:  return f"v{s.s1} {s.alu} [{s.mem:#06x}]"
        if s.op == ALUV:  return f"v{s.s1} {s.alu} v{s.s2}"
        if s.op == MOVE:  return f"move v{s.s1}"
        if s.op == NEG:   return f"neg v{s.s1}"
        if s.op == STORE: return f"store v{s.s1} -> [{s.mem:#06x}]"
        return s.op
...
def decode1(b, i):
    ...
    if need is None or i + need > len(b):
        dlen = None
    elif base == 0x06: dlen, d = 2, struct.unpack_from('<H', b, i+2)[0]
    elif base == 0x46: dlen, d = 1, struct.unpack_from('<b', b, i+2)[0]
    else:              dlen, d = 2, struct.unpack_from('<h', b, i+2)[0]
    if dlen is not None:
        ln = 2 + dlen
        if i + ln > len(b): return None
        if op == 0x8B: return ('ld', pair, 0, half, None, d, ln, base, dlen)
        if op == 0x89: return ('st', pair, 0, half, None, d, ln, base, dlen)
        for lo,(hi,w,nm) in PAIRS.items():
            if op == lo or op == hi:
    ...
    if (m & 0xC0) == 0xC0:                      # register to register
        ...
        if rm > 3: return None
        sp, sh = REG[rm]
        if sh != half: return None              # halves must match
        if op == 0x8B: return ('mv', pair, sp, half, None, None, 2, 0, 0)
        for lo,(hi,w,nm) in PAIRS.items():
            if op == lo or op == hi:
...
def lift(b, start, end):
    ...
    vals, stores, cur, notes = [], [], {0:None, 1:None}, []
    i = start
    def new(v):
        vals.append(v); return len(vals)-1
    while i < end:
        ...
        for pr,(lo,hi) in ((0,(0xD8,0xDA)),(1,(0xD9,0xDB))):
            if (i+7 <= min(end, len(b)) and b[i]==0xF7 and b[i+1]==lo
                    and b[i+2]==0x83 and b[i+3]==(0xD0|(hi&7)) and b[i+4]==0
                    and b[i+5]==0xF7 and b[i+6]==hi and cur[pr] is not None):
        ...
        if i >= end: break
        a = decode1(b, i)
        if a is None: cur[0]=cur[1]=None; i += 1; continue
        kind, pair, sp, half, alu, mem, ln, base, dlen = a
        nb = decode1(b, i+ln)
        ok = False
        if nb is not None:
            k2,p2,s2,h2,alu2,mem2,ln2,base2,dlen2 = nb
            same = (k2==kind and p2==pair and s2==sp and h2 is not None
                    and h2 != half and base2==base)
            if same and kind in ('ld','st','op'):
                ...
                if ok:
                    if kind=='ld':
                        ...
                        if cur[pair] is None: ok=False
                        else: cur[pair] = new(Value(ALUM, alu=PAIRS[lo_a][2],
                                s1=cur[pair], mem=lo_d, at=i, end=i+ln+ln2,
                                pair=pair, base=base, dlen=dlen))
                    else:
                        if cur[pair] is None: ok=False
                        else: stores.append(new(Value(STORE, s1=cur[pair],
                                mem=lo_d, at=i, end=i+ln+ln2, pair=pair, base=base, dlen=dlen)))
                ...
                if kind=='mv':
                    ...
                    if cur[sp] is None: ok = False
                    else:
                        cur[pair] = new(Value(MOVE, s1=cur[sp], at=i,
                                              end=i+ln+ln2, pair=pair,
                                              src_pair=sp))
                        ok = True
                else:
                    lo_a = alu if half==0 else alu2
                    if cur[pair] is None or cur[sp] is None: ok=False
                    else:
                        cur[pair] = new(Value(ALUV, alu=PAIRS[lo_a][2],
                                s1=cur[pair], s2=cur[sp], at=i, end=i+ln+ln2,
                                pair=pair, src_pair=sp))
                        ok = True
        if ok: i += ln + ln2
        else:  cur[0]=cur[1]=None; i += 1
    return vals, stores
...
def report(vals, stores):
    ...
    for n,v in enumerate(vals):
        if v.op == STORE: mem[v.mem] = v.s1
        elif v.op == ALUM and v.mem in mem: fwd += 1
    return dict(values=len(vals), stores=len(stores),
                alu_v=sum(1 for v in vals if v.op==ALUV),
                forwardable=fwd)
...
if __name__ == "__main__":
    ...
    print()
    for k,x in report(vals, stores).items(): print(f"   {k:14} {x}")
...
def needed(vals, regs=None):
    ...
    need = [False] * len(vals)
    for n, v in enumerate(vals):
        if v.op == STORE: need[n] = True
    if regs is None: regs = regions(vals)
    for reg in regs:
        last = {}
        for n in reg:
            if vals[n].op != STORE: last[vals[n].pair] = n
        for n in last.values(): need[n] = True
    for n in range(len(vals) - 1, -1, -1):
        if not need[n]: continue
        for s in (vals[n].s1, vals[n].s2):
            if s is not None: need[s] = True
    return need
...
def sizeof(v, need):
    ...
    if v.op == LOAD:  return 3 + v.dlen                   # 66 8B modrm d
    if v.op == ALUM:  return 3 + v.dlen                   # 66 op modrm d
    if v.op == ALUV:  return 3                            # 66 op C1 etc
    if v.op == MOVE:  return 3                            # 66 8B C1 etc
    if v.op == NEG:   return 3                            # 66 F7 D8 etc
    if v.op == STORE: return 3 + v.dlen
    return 0
...
def plan(vals, need):
    ...
    for reg in regions(vals):
        ...
        out.append(dict(first=reg[0], last=reg[-1], at=vals[reg[0]].at,
                        before=before, after=after, dead=moves,
                        fits=(after <= before), n=len(reg)))
    return out
...
def encode(v):
    ...
    o = OPC.get(v.alu, 0)
    disp = (struct.pack('<b', v.mem) if v.dlen == 1
            else struct.pack('<h', v.mem) if v.mem < 0
            else struct.pack('<H', v.mem)) if v.mem is not None else b''
...
FIXUP = {0: bytes([0x66,0x8B,0xD0, 0x66,0xC1,0xEA,0x10]),   # edx=eax, shr 16
         1: bytes([0x66,0x8B,0xD9, 0x66,0xC1,0xEB,0x10])}   # ebx=ecx, shr 16

def emit_region(vals, need, reg):
    """Bytes for one region, plus the jump over whatever is left."""
    out = b''
    for n in reg:
        if need[n]: out += encode(vals[n])
    for p in sorted({vals[n].pair for n in reg if need[n]}):
        out += FIXUP[p]
    before = vals[reg[-1]].end - vals[reg[0]].at
    slack = before - len(out)
    if slack < 0: return None
    if slack == 1: out += b'\x90'
    elif slack >= 2: out += bytes([0xEB, slack-2]) + b'\x90'*(slack-2)
    return out
