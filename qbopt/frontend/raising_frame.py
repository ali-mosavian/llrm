...
from iced_x86 import FlowControl, Mnemonic, OpKind, Register

from qbopt.abi import runtime
from qbopt.frontend.blocks import event_enabled
from qbopt.frontend.declen import INFO, WRITES
from qbopt.frontend.extent import ENTRY, has_header
...
def _private(blocks):
    for block in blocks:
        for decoded in block.insns:
            insn = decoded.insn
            if any(insn.op_kind(index) == OpKind.REGISTER and insn.op_register(index) in POINTERS
...
def _after(decoded, depth, contracts):
    ...
    if insn.mnemonic == Mnemonic.CALL:
        ...
        if (contract is None or not contract.established or runtime.barrier(contract)
            or contract.cleanup is None or contract.control is not runtime.Control.RETURNS
...
def annotated(body, found, blocks, contracts):
    ...
    if (body.entry != ENTRY or layout is None or event_enabled(found)
        or any(contract.error_handling for contract in contracts.values())):
        return body
    floor, size = layout
    ...
    def rewrite(op):
        ...
        own = (private and op.kind is mir.Kind.CALL and contract is not None
               and contract.writes is runtime.Memory.OWN and contract.reads is runtime.Memory.OWN
               and contract.inputs is not None and not contract.inputs & {runtime.Reg.BP, runtime.Reg.SP})
        ...
        return replace(op, loads=tuple(map(reference, op.loads)), stores=tuple(map(reference, op.stores)),
