...
def initialized(body: mir.MirBody, found) -> mir.MirBody:
    ...
    if not pools:
        return body
    far_strings = {index for index, segment in enumerate(omf.segments(found.records))
    ...
    def annotate(op):
        if op.kind is not mir.Kind.CALL or not protected:
            return op
        contract = runtime.contract(found.calls.get(op.at, ""))
        if runtime.barrier(contract) or contract.writes is not runtime.Memory.OWN:
            return op
        return replace(op, stores=tuple(replace(ref, excludes=protected) if ref.addr is None else ref
                                        for ref in op.stores))
    return replace(body, initial=tuple(values.items()),
...
def _numeric_ranges(values, data, ambiguous, fixups, escaped, far_strings=frozenset()):
    ...
    for ref in values:
        ...
        for segment, offset in escaped:
            ...
            if (pointer is None or pointer.index != index
                or (index, offset) not in data or (index, offset + 1) not in data
                or (index, offset) in ambiguous or (index, offset + 1) in ambiguous):
                break
...
def _far_descriptor(index, offset, data, ambiguous, fixups, far_strings):
    ...
    if (base is None or base.index != pointer.index or base.disp != 0 or indirect is None
        or indirect.index != pointer.index or indirect.disp != pointer.disp + 2):
        return None
    ...
    if any((pointer.index, byte) not in data or (pointer.index, byte) in ambiguous
...
def _relocation(segment, at, kind, data, fixups):
    ...
    fields = [fixup for fixup in fixups if fixup.seg == segment
              and fixup.offset < at + 2 and at < fixup.offset + widths.get(fixup.loc, 4)]
    if (len(fields) != 1 or fields[0].offset != at or fields[0].loc != kind or fields[0].target != "segment"
