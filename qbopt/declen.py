...
def one_byte_opcodes() -> tuple[int, ...]:
    ...
    put([0xC3, 0xCB], FLOW)
    put([0xC2, 0xCA], I16 | FLOW)
    put([0x9A, 0xEA], FLOW | 0x100 & 0xFF)  # far ptr -- handled specially below
    table[0x9A] = table[0xEA] = FLOW | I16 | IZ  # seg:off = imm16 after an imm16/32
    put([0xCD], I8 | FLOW)
    put([0xCF], FLOW)  # int / iret
...
def two_byte_opcodes() -> tuple[int, ...]:
    ...
    put(range(0x80, 0x90), IZ | FLOW)  # jcc rel16/32
...
def length(b: bytes, i: int, opsize: int = 16, addrsize: int = 16) -> int | None:
    ...
    if op in GROUP_F:
        if i >= len(b):
            return None
        imm = M | GROUP_F[op] if (b[i] >> 3) & 7 in (0, 1) else M
