...
def _removed(op: mir.Op) -> bool:
    ...
    if (
        ...
        or op.node is not None
        or op.made is not None
        or op.raised is not None
    ):
    ...
    def slots(values, machine):
        ...
        for value, operand in zip(values, machine, strict=True):
            if isinstance(value, mir.Held) and value.width == 10:
                if (
                    not isinstance(operand, mir.Opaque)
                    or not operand.name.startswith("st")
...
def operation(op: mir.Op) -> mir.Op:
    ...
    variables = {
        arg.value.variable for arg in (*origin.inputs, *origin.outputs) if isinstance(arg, mir.Held) and arg.width == 10
    }
    ...
    def operands(current, before, machine):
        ...
        for arg, original, old in zip(current, before, machine):
            if isinstance(original, mir.Held) and original.width == 10:
                if not isinstance(arg, mir.Held) or arg.width != 10 or arg.value.variable != original.value.variable:
...
def _forwarded_inputs(op):
    ...
    return {
        index
        for index, (arg, old, format, before) in enumerate(
            zip(op.args, origin.inputs, op.floating.inputs, origin.semantics.inputs)
        )
        if isinstance(old, mir.Cell)
        and isinstance(arg, mir.Held)
        and arg.width == 10
        and before in (Format.BINARY32, Format.BINARY64)
        and format is Format.EXTENDED80
    }
...
def checked(body: mir.MirBody) -> None:
    ...
    for block in body.blocks:
        ...
        if any(
            op.floating_origin is None and any(value.variable in floating for value in (*op.uses, *op.defines))
            for op in block.ops
        ):
    ...
    for block in body.blocks:
        ...
        for op in typed:
            ...
            if forwarded:
                formats = tuple(
                    origin.semantics.inputs[index] if index in forwarded else format
            ...
            for current, before in ((op.args, origin.inputs), (op.results, origin.outputs)):
                ...
                if len(current) != len(before) or any(
                    (type(arg) is not type(old) and index not in allowed)
