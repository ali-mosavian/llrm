...
def signed_multiply(cpu: str | targets.Profile, width: int, *, full: bool = False) -> Clocks | None:
    if width not in (2, 4):
        return None
    match targets.profile(cpu).name:
        case "386":
            return Clocks(9, 22 if width == 2 else 38)
        case "486":
            return Clocks(13, 26 if width == 2 else 42)
        case "P5":
...
def signed_divide(cpu: str | targets.Profile, width: int) -> Clocks | None:
    if width not in (2, 4):
        return None
    match targets.profile(cpu).name:
        case "386" | "486":
            clocks = 27 if width == 2 else 43
        case "P5":
            clocks = 30 if width == 2 else 46
        case _:
            return None
    return Clocks(clocks, clocks)
