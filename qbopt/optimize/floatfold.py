...
def _checked(op):
    kind = (
        mir.Kind.NOTHING
        if op.floating is not None and op.floating.exceptions is Exceptions.DEFERRED
        else mir.Kind.FCHECK
    )
    return replace(
        op,
        kind=kind,
...
def stored(body: mir.MirBody, facts: dict) -> mir.MirBody:
    ...
    for block in body.blocks:
        ...
        for op in block.ops:
            if (
                op.kind is mir.Kind.FSTORE
                and op.floating is not None
                and op.floating.result in {Format.BINARY32, Format.BINARY64}
                and not op.barrier
                and len(op.stores) == len(op.args) == 1
                and not op.defines
...
def discarded(body: mir.MirBody, converted: dict) -> mir.MirBody:
    ...
    for block in body.blocks:
        ...
        for index, conversion in enumerate(ops):
            if (
                conversion.kind is not mir.Kind.FSTORE
                or conversion.stores
                or conversion.barrier
            ...
            if (
                ...
                or result.value not in converted
                or reads[result.value]
                or not isinstance(source, mir.Held)
                or source.width != 10
                or reads[source.value] != 1
            ):
                continue
            ...
            if (
                load is not None
                and load.kind is mir.Kind.FLOAD
                and not load.barrier
                and not load.stores
                and load.results == (source,)
                and len(load.args) == 1
                and isinstance(load.args[0], mir.Cell)
