"""Place semantic operations on explicit conditional edges, not on either arm."""
...
def explicit(block: mir.MirBlock, target: int) -> bool:
    return (len(block.succ) == 2 and target in block.succ and bool(block.ops)
            and block.ops[-1].kind is mir.Kind.BRANCH and block.ops[-1].target == target)
...
def split(body: mir.MirBody, source: int, target: int, label: int,
    ...
    if parent is None or not explicit(parent, target) or body.block(label) is not None:
        raise ValueError("edge split does not identify a fresh explicit branch edge")
