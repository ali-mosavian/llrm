...
def optimized(
    ...
    *,
    ...
    if (
        len(loops.loops(result.blocks, result.entry)) >= len(loops.loops(body.blocks, body.entry))
    ...
    for loop in loops.loops(closed.blocks, closed.entry):
        ...
        for block in closed.blocks:
            ...
            if (
                ...
                or branch.defines
                or branch.results
                or mir.partial(branch)
                or compare.loads
                or compare.stores
                or compare.barrier
                or compare.floating
                or mir.partial(compare)
                or compare.stack
                or not all(isinstance(arg, (mir.Held, mir.Const)) for arg in compare.args)
                or any(
                    owners.get(value) in loop.body or (value in owners and owners[value] not in dominators[entry])
                    for value in compare.uses
                )
            ):
                continue
...
def _specialized(body, copied, loop, entry, selected, compare, branch):
    ...
    definitions = {
        value.id: replace(value, id=next_id + index, variable=next_variable + index, version=1)
    ...
    guard = replace(
        compare,
        at=anchor,
        defines=tuple(definitions[value.id] for value in compare.defines),
    ...
    dispatch = replace(
        ssa.substituted(branch, definitions),
        at=anchor,
    ...
    parent = replace(
        parent,
        ops=(*(parent.ops[:-1] if replaces_jump else parent.ops), guard, dispatch),
    ...
    for block in copied.blocks:
        if block.at == entry:
            ...
            phis = tuple(
                replace(phi, incoming={**phi.incoming, cloned_latch: residual.incoming[cloned_latch]})
        ...
        if block.at == cloned_latch:
            block = replace(
                block,
                succ=(cloned_header,),
                ops=tuple(replace(op, target=cloned_header) if op.target == loop.header else op for op in block.ops),
            )
