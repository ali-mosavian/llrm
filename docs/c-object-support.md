...
qbopt on them is not yet equivalent to optimizing them. Assembly objects remain
...
WAIT/opcode bytes, not ordinary operand fields. `frontend/fppatches.py` now
...
`backend/nativeframe.py` now recognizes the entry footprint. R_WALK's public
...
for R_EMIT_ENTITIES (28 bytes). `abi/nativecalls.py` supplies those facts;
...
`optimize/ivshare.py` reconstructs a related recurrence from an equal-step
...
`backend/addressforms.py` follows word-sized copies and constant additions
...
`model/mir.py` and the full integration/commit gate remain outstanding.
...
`analysis/frameescape.py` now propagates explicit frame origins through
...
`objectfile/addends.py` now moves code OFF16 addends into FIXUPP before
