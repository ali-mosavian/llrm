...
and abstract assembly listing are in `docs/measurement/targets.md`.
...
[Current coverage](../measurement/target-coverage-current.md) now replaces the stale status
...
already recorded in `docs/qrender/contracts.md`; it supplies no DF/DS proof. Do not
...
block totals are in docs/measurement/targets.md; neither target is a fraction of our code.
...
listing in docs/measurement/targets.md: input 64, arithmetic/final stores 51, output 102.
...
computation 226, output 114, total 710. `docs/measurement/targets.md` records the audit
...
certifies these unestablished references. `docs/measurement/targets.md` records the
...
updated together, with no flat compatibility wrappers. `../architecture/source-layout.md`
...
`../semantics/floating-environment.md` records the sources, decoded control word, limits of
...
all ten iterations without assuming a rounding mode. `docs/measurement/targets.md` now
...
branches. Its new complete hand listing in `docs/measurement/targets.md` keeps all twelve
...
reference. `docs/measurement/targets.md` derives the full source states: PDS/VBDOS require
...
`docs/qrender/contracts/qrender-r-sweep.json`; UGL cleanup-only facts are independently
...
is the whole of the 1.02x. The 3x to 17x in `docs/measurement/targets.md` is re-selection
...
which is `docs/measurement/targets.md`'s own reference term for term -- `s = (20 *
...
`../measurement/target-coverage-current.md` says of its own numbers: "This is not an
