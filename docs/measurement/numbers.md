...
Every row carries what produced it. See `docs/measurement/readme.md` for what each
...
see [the change and its before/after assembly](../optimizations/invariant-shift.md).
...
speedup; `docs/measurement/timing-audit.md` records the stage-level diagnosis and fix.
...
(1404 found, 1382 taken): commit 3 (docs/optimizations/residue.md's G and H) never adds or
...
`docs/optimizations/residue.md`'s patterns, both plain codegen bugs rather than new
