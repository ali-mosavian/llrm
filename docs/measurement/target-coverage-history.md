# Historical target coverage and floating-point investigations

These are retained checkpoints, not current metrics. See
[the current snapshot](target-coverage-current.md) for the latest measured state.

Local dead-value update: FPDEEP now costs PDS 12287 (11.31x), QB 12555
(11.56x), VBDOS 12267 (11.30x). The target remains 1086. See
`docs/optimizations/local-dead-values.md` for the removed return-half reconstruction,
...
14481, 14699 and 14411 respectively. See `docs/optimizations/float-integer-values.md`;
...
printing sequence in `docs/measurement/targets.md`. After finite-array reuse and stack
...
See `docs/optimizations/finite-array-reuse.md` for before/after assembly and verification.
...
strict-FP caveats in `docs/semantics/floating-environment.md` remain applicable.
