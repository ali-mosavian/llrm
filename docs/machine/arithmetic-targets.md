...
486/P5/P6 and the other existing timing profiles use `crates/llrm-core/src/cycles/timings.rs`.
...
`crates/llrm-core/src/backend/division.rs` derives the multiplier with LLVM's
...
The old midpoint comparisons have been withdrawn. `crates/llrm-core/src/backend/timing.rs`
...
still need the remaining timing audit; see `../measurement/timing-audit.md`.
