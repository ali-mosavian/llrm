...
`tools/stages.py` dumps nothing before the raise, so Phase 5's object core
has no stage to diff. `ported (tests, BC harness)` there means its Python tests
are ported and a Rust-vs-Python dump of every pre-raise fact wholeseg computes
matched on every object under `fixtures/`.
...
bytes or refusal. `draft (awaits raise)` is ported but cannot be diffed end to
end until `mir.bodies` lands; `wholeseg._raised` is the seam.
...
|---|---|---:|---|---|---|
| - | `qbopt/legacy/calls.py` | 839 | `src/legacy/calls.rs` | partial (absorb half) | test_calls, test_runtime, test_select, test_transform |
...
| 2 | `qbopt/flow.py` | 198 | `src/flow.rs` | ported (C path) | test_allocation, test_coalesce, test_cpu_profile, test_flow, test_jumps, test_lir_verify, test_native_frame, test_parcopy, test_raising_copies, test_spiller, test_wholeseg |
