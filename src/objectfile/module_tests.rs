...
pub(crate) fn objects() -> Vec<PathBuf> {
...
/// `corpus.loaded`.
pub(crate) fn loaded(path: impl AsRef<Path>) -> Option<Module> {
    of(&omf::read(path).unwrap())
}

/// `Module(records, seg, name, code, start, end)` with every other field defaulted.
pub(crate) fn bare(found: &Module, code: Vec<u8>, start: i64, end: i64) -> Module {
    Module {
