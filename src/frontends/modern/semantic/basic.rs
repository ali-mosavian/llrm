...
//!
//! A library for BASIC links without the modern runtime, whose start-up and
//! heap BASIC's own replace: a statement that calls it is refused.

use super::*;
use crate::frontends::modern::syntax::{Adapter, Basic};
...
impl FunctionCompiler<'_> {
    ...
    /// Refuses the modern runtime routine the code since call `since` calls,
    ...
    pub(super) fn refuse_runtime(&self, since: usize, span: Span) -> Result<(), Diagnostic> {
        ...
        match called {
            Some(routine) => Err(Diagnostic::new(
                span,
                format!(
                    "this calls the modern runtime ({routine}), which a library for {} links without; \
