...
/// `main.mod` and `geo.mod` on disk, and the messages opening `main` in an editor.
struct Program {
...
impl Program {
    fn new() -> Self {
        ...
        for (name, text) in [("main", MAIN), ("geo", GEO)] {
            std::fs::write(root.path().join(name).with_extension(crate::frontends::modern::modules::EXTENSION), text).expect("written");
        }
        Self { root }
    }

    fn uri(&self, module: &str) -> String {
        documents::uri(&crate::frontends::modern::modules::file(self.root.path(), module))
    }
