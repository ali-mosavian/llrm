...
";
    ...
    let checks = |hir: &str| hir.matches(&format!("\"callee\":\"{}\"", crate::abi::modern::ERROR_BOUNDS)).count();
...
fn a_type_with_display_prints_and_formats_through_it() {
    ...
    // outer one was building: "M$PEND without M$PBEG".
...
fn the_readings_example_smooths_peeks_and_reports_generators() {
    let source = std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/docs/examples/readings.mod")).expect("the example");
    assert_eq!(
        output_without_leaks(&source),
...
fn the_planets_example_prints_floats_as_the_shortest_text_that_reads_back() {
    let source = std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/docs/examples/planets.mod")).expect("the example");
    assert_eq!(
        output_without_leaks(&source),
...
#[test]
fn a_library_for_basic_refuses_the_modern_runtime_and_misplaced_adapters() {
    ...
    assert!(printing.contains("modern runtime (M$PI2)") && printing.contains("qb45"), "{printing}");
    ...
    assert!(refused_with_imports(checked).contains("M$EBND"), "{}", refused_with_imports(checked));
...
fn qb45_exports_are_declared_as_basic_procedures() {
    use super::declarations::{Language, declarations};
    let source = std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/docs/examples/basic/sortlib.mod")).expect("the example");
    let module = super::modules::load(&source, &mut |name| Err(format!("{name} is not supplied"))).expect("loads");
...
fn a_slice_past_the_end_or_reversed_panics() {
    ...
    let checks = |source: &str| super::compile(source, "t").expect("compiles").matches(&format!("\"callee\":\"{}\"", crate::abi::modern::ERROR_BOUNDS)).count();
...
fn the_pipeline_example_streams_words_through_escaping_stages() {
    let source = std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/docs/examples/pipeline.mod")).expect("the example");
    assert_eq!(
        output_without_leaks(&source),
        "skipped x\nmean 12\nmean 18\nmean 23\nskipped err\nmean 28\nmean 21\nmean 15\nclosed feed\nskipped x\nalarm at 23\nclosed alarm\ndone\n"
    );
...
fn a_move_in_a_generator_body_is_checked_however_the_body_is_compiled() {
    ...
    // host run failed with "M$PS of null".
