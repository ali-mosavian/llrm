...
/// The program at `source`. In process: `cargo run` of `modernfront` rebuilt
...
pub fn parsed(source: &Path, dump: Option<&Path>) -> Result<model::Program, FrontendError> {
    ...
    codec::decode(&text).map_err(|error| FrontendError(format!("the modern frontend emitted invalid HIR: {error}")))
}

/// A diagnostic at `path`, as `modernfront` reports it.
