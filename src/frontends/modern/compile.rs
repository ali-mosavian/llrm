...
use crate::hir::lower::{DGROUP, Lowered};
...
use crate::optimize::interprocedural as whole;
...
// use their compact, mangled ABI names (for example `M$PS`), and an exported
...
pub fn assembled(
    ...
    // Call constants come from MIR alone: modern has no source-time table of them.
...
/// called; every modern call is far, so that is safe.
