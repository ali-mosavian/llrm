...
impl Program {
    fn new() -> Self {
        ...
        for (name, text) in [("main", MAIN), ("geo", GEO)] {
            std::fs::write(root.path().join(name).with_extension(crate::frontends::nib::modules::EXTENSION), text).expect("written");
        }
        Self { root }
    }

    fn uri(&self, module: &str) -> String {
        documents::uri(&crate::frontends::nib::modules::file(self.root.path(), module))
    }
