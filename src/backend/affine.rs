...
pub(crate) type Terms = Vec<(Register, i64)>;

/// Whether `one` has no effect beyond its operands.
pub(crate) fn plain(one: &Insn) -> bool {
    one.what.is_some()
...
pub(crate) fn step(one: &Insn, cpu: &Profile) -> Option<(Reg, Step, i64)> {
    if !plain(one) {
        return None;
    }
...
pub(crate) fn form(terms: &[(Register, i64)], disp: i64, scales: &BTreeSet<i64>) -> Option<Address> {
