...
fn _color_slots(
    ...
    for value in values {
        ...
        let had = frame.capacities.get(&home).copied().unwrap_or(frames::WORD);
        frame.capacities.insert(home, had.max(capacity));
    }
    let mut pending: Vec<u32> =
        values.iter().copied().filter(|value| !frame.slots.contains_key(&SlotKey::from(*value))).collect();
    pending.sort_by_key(|value| (-i64::from(widths[value].max(frames::WORD)), *value));
    for value in pending {
        let width = widths[&value];
        let capacity = width.max(frames::WORD);
...
fn _existing_colors(body: &LirBody, frame: &mut Frame) -> Vec<(i64, u32, Vec<Interval>)> {
    ...
    let mut capacities: IndexMap<i64, u32> = homes
        .iter()
        .map(|home| (*home, frame.capacities.get(home).copied().unwrap_or(frames::WORD)))
        .collect();
    ...
    let mut slot = |operand: &Loc, capacities: &mut IndexMap<i64, u32>| -> Option<u32> {
        ...
        if let Some(found) = pseudo.get(&home) {
            ...
            capacities.insert(home, had.max(cell.width).max(frames::WORD));
            return Some(*found);
        }
    ...
    for home in homes {
        ...
        frame.capacities.insert(home, capacities[&home]);
...
///
/// `select.emit` is `port/select`'s; until it merges this refuses.
fn _encodable(what: &Semantics) -> Result<bool, Error> {
    ...
    let _ = probe;
    Err(Error::Value("not yet ported: qbopt.backend.select.emit".to_owned()))
}
