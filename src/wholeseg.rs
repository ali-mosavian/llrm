...
//!
//! `mir.bodies`, the raise, is another unit's port; `_raised` is the one
//! seam that calls it.
...
use crate::flow;
use crate::frontend::blocks::{self as split, Block, CodeMap, code_map};
use crate::frontend::{extent, fppatches};
use crate::model::ir;
...
/// Python's `RaisedBodies`: the raised bodies and their provenance.
pub struct RaisedBodies {
    pub values: Vec<(String, MirBody)>,
    pub source: SourceMap,
    pub hints: IndexMap<i64, AllocationHints>,
}

/// `mir.bodies(found, blocks, contracts, ...)`: the raise.
///
/// The BC raise is being ported in parallel and is not here yet, so this
/// raises nothing, which is what Python's `bodies` answers for a module it
/// cannot decode: every object refuses with "no bodies were raised".
pub fn _raised(
    found: &Module,
    blocks: &[Block],
    ...
    let _ = (blocks, contracts, basic_semantics, bounds_checks);
    RaisedBodies { values: Vec::new(), source: SourceMap::from_module(found), hints: IndexMap::default() }
}
...
pub fn _rebuilt(
    ...
    let raised = _raised(&found, &blocks, &contracts, basic_semantics, bounds_checks);
    let mut bodies: Vec<(String, Rc<MirBody>)> =
        raised.values.into_iter().map(|(name, body)| (name, Rc::new(body))).collect();
    ...
    if optimise {
        let block_views: Vec<Arc<dyn std::any::Any + Send + Sync>> =
            blocks.iter().map(|block| Arc::new(block.clone()) as Arc<dyn std::any::Any + Send + Sync>).collect();
        ...
        for (name, body) in bodies {
            ...
            // `found` stays None: flow.optimized takes it as a Send + Sync
            // Any, which Module (it holds Rc records) cannot be, and its only
            // reader, observers.exposure, is not ported.
