...
use crate::old::hir::{LowerError, Program, RuntimeProfile, Storage};
use crate::old::ir;
use crate::old::object::omf::file::{File as OmfFile, FileError};
use crate::old::object::omf::write::WriteError as OmfWriteError;
use crate::old::target::x86::{
...
pub enum Error {
    ...
    QbStatementTableLayout(crate::old::mc::LayoutError),
...
pub fn compile_wcc_capture_unit(
    ...
    crate::old::hir::lower_to_ir_with_array_order(&module, crate::old::hir::ArrayOrder::RowMajor)
        .map_err(Error::Lower)
}
...
pub fn lower_qb_to_ir(program: &Program) -> Result<ir::Module, Error> {
    ...
    crate::old::hir::lower_to_ir_with_array_order(&extracted.module, program.array_order)
        .map_err(Error::Lower)
}
...
pub fn lower_ir_to_machine(
    module: &ir::Module,
) -> Result<crate::old::codegen::machine::MachineModule, Error> {
    let machine = crate::old::target::x86::select_module(module).map_err(Error::Selection)?;
    crate::old::target::x86::verify_machine(&machine).map_err(Error::Machine)?;
    Ok(machine)
}
...
pub fn lower_qb_to_machine(program: &Program) -> Result<QbMachine, Error> {
    ...
    let ir = crate::old::hir::lower_to_ir_with_array_order(&physical, program.array_order)
        .map_err(Error::Lower)?;
    ...
    for function in &mut machine.functions {
        ...
        if source_function.name != "__main" {
            ...
            let expanded = crate::old::target::x86::expand_basic_runtime(
                function,
                runtime,
        ...
        *function =
            crate::old::target::x86::materialize_far_call_clobbers(function).map_err(|error| {
                Error::CallClobber {
                    function: source_function.name.clone(),
                    error,
                }
            })?;
        *function = crate::old::target::x86::allocate_x87_stack(function).map_err(|error| {
            Error::FloatAllocation {
                function: source_function.name.clone(),
                error,
            }
        })?;
        *function = crate::old::target::x86::expand_x87_truncation_with_synchronization(
            function,
...
pub fn allocate_qb_machine(selected: &QbMachine) -> Result<QbMachine, Error> {
    ...
    for function in &mut allocated.module.functions {
        let allocation = crate::old::target::x86::allocate_registers_with_spills(function)
        ...
        if let Some(previous) = allocated.frames.get(&rewritten.id) {
            let refreshed =
                crate::old::target::x86::refresh_basic_runtime_frame(&rewritten, previous)
                    .map_err(|error| Error::BasicAbi {
        ...
        rewritten =
            crate::old::target::x86::forward_frame_reloads(&rewritten, &allocation.assignment);
        ...
        *function = apply_assignment(&rewritten, &allocation.assignment).map_err(|error| {
        ...
        if let Some(frame) = allocated.frames.get(&function.id) {
            *function = crate::old::target::x86::materialize_frame_indices(function, frame)
    ...
    crate::old::target::x86::verify_machine(&allocated.module).map_err(Error::Machine)?;
    Ok(allocated)
}

/// Allocates selected QB Machine IR, finalizes its target ABI, and lowers it to MC.
pub fn lower_qb_machine_to_mc(selected: &QbMachine) -> Result<crate::old::mc::MCModule, Error> {
    let mut allocated = allocate_qb_machine(selected)?;
    for function in &mut allocated.module.functions {
        *function = crate::old::target::x86::expand_allocated_segmented_memory(function).map_err(
        ...
        *function =
            crate::old::target::x86::expand_allocated_word_merges(function).map_err(|error| {
        ...
        *function =
            crate::old::target::x86::expand_allocated_basic_abi(function).map_err(|error| {
                Error::BasicAbiExpansion {
                    function: function.name.clone(),
                    error,
                }
            })?;
        *function = crate::old::target::x86::place_and_thread(
            function,
            &crate::old::target::x86::ControlFlowFacts::default(),
        )
    ...
    crate::old::target::x86::verify_machine(&allocated.module).map_err(Error::Machine)?;
    crate::old::target::x86::lower_allocated_module(&allocated.module).map_err(Error::Mc)
}
...
pub fn lower_c_to_machine(module: &ir::Module) -> Result<CMachine, Error> {
    ...
    for function in &mut machine.functions {
        *function =
            crate::old::target::x86::materialize_c_call_clobbers(function).map_err(|error| {
                Error::CallClobber {
                    function: function.name.clone(),
                    error,
                }
        ...
        *function = crate::old::target::x86::expand_x87_truncation_with_synchronization(
        ...
        let plan =
            crate::old::target::x86::plan_c_frame(function).map_err(|error| Error::CFrame {
                function: function.name.clone(),
                error,
            })?;
...
pub fn allocate_c_machine(selected: &CMachine) -> Result<CMachine, Error> {
    ...
    for function in &mut allocated.module.functions {
        ...
        let frame =
            crate::old::target::x86::plan_c_frame(&rewritten).map_err(|error| Error::CFrame {
                function: rewritten.name.clone(),
                error,
            })?;
        ...
        *function = apply_assignment(&rewritten, &allocation.assignment).map_err(|error| {
        ...
        if let Some(frame) = allocated.frames.get(&function.id) {
            *function = crate::old::target::x86::materialize_frame_indices_with_layout(
                function,
                frame.layout(),
            )
            .map_err(|error| Error::FrameIndices {
...
pub fn lower_c_machine_to_mc(selected: &CMachine) -> Result<crate::old::mc::MCModule, Error> {
    ...
    for function in &mut allocated.module.functions {
        ...
        *function =
            crate::old::target::x86::expand_allocated_c_abi(function, frame).map_err(|error| {
                Error::CAbiExpansion {
                    function: function.name.clone(),
                    error,
                }
...
pub fn encode_x86_mc(module: &crate::old::mc::MCModule) -> Result<crate::old::mc::MCModule, Error> {
    crate::old::target::x86::relax_and_encode_jumps(module).map_err(Error::McEncoding)
}
...
pub fn write_x86_omf(
    module_name: &[u8],
    module: &crate::old::mc::MCModule,
) -> Result<Vec<u8>, Error> {
    let object =
        crate::old::target::x86::lower_to_omf(module_name, module).map_err(Error::X86Omf)?;
    crate::old::object::omf::write::to_bytes(&object).map_err(Error::OmfWrite)
}
...
pub fn write_c_omf(
    ...
    let object = crate::old::target::x86::lower_to_omf_with_dgroup(module_name, module)
...
pub fn write_qb_omf(program: &Program, module_name: &[u8]) -> Result<Vec<u8>, Error> {
    ...
    let text_sections = lowered
        .sections
        .iter()
        .filter(|section| section.kind == crate::old::mc::SectionKind::Text)
    ...
    let table = qb::statement_mc::append_statement_table(
        ...
        *text_section,
        &[],
        crate::old::target::x86::X86FixupKind::Absolute16.into(),
    )
    ...
    let layout =
        crate::old::mc::layout(&encoded, |_| None).map_err(Error::QbStatementTableLayout)?;
    let crate::old::mc::SymbolLayout::Defined { offset, .. } = layout
        .symbols
    ...
    let generic =
        crate::old::target::x86::lower_to_omf(module_name, &encoded).map_err(Error::X86Omf)?;
...
fn temporary_string_slots(
    function: &crate::old::hir::Function,
    string_types: &BTreeSet<crate::old::hir::TypeId>,
) -> Result<u16, Error> {
    let count = function
        .places
        .iter()
...
mod tests {
    ...
    use crate::old::ir;
    use crate::old::mc::{MCFragment, MCOperand, SymbolDefinition};
    use crate::old::object::omf::file::File as OmfFile;
    use crate::old::object::omf::module::DecodedModule;
    use crate::old::object::omf::record::Record;
    use crate::old::target::x86::{X86FixupKind, X86Opcode, X86Register};
    ...
    fn qb_procedure_preserves_byref_frames_and_dx_ax_long_abi() {
        ...
        assert!(matches!(
            ...
            [
                crate::old::codegen::machine::FrameObject {
                    size: 2,
    ...
    fn qb_emission_fixture_reaches_its_measured_object_envelope() {
        ...
        assert_eq!(
            decoded.declarations.groups[0]
                ...
                .map(|member| match member {
                    crate::old::object::omf::declarations::GroupMember::Segment {
                        segment_index,
                    } => {
                        *segment_index
                    }
        ...
        assert!(decoded.fixups.iter().any(|fixup| {
            fixup.segment_index == 1
                ...
                && fixup.location == crate::old::object::omf::fixups::Location::Offset16
                && fixup.frame.method == crate::old::object::omf::fixups::FrameMethod::Target
                && fixup.target.method == crate::old::object::omf::fixups::TargetMethod::Segment
                && fixup.target.datum == 1
        }));
        assert!(decoded.fixups.iter().any(|fixup| {
            fixup.segment_index == 12
                && fixup.location == crate::old::object::omf::fixups::Location::Pointer16_16
        }));
    }
    ...
    fn verified_integer_ir_reaches_x86_machine_ir() {
        ...
        assert_eq!(
            machine.functions[0].blocks[0]
                ...
                .unwrap()
                .opcode,
            crate::old::target::x86::X86Opcode::ReturnNear.machine_opcode()
        );
    }
}
