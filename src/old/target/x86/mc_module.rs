...
fn data_relocation_kind(
    data: MachineDataObjectId,
    relocation: &crate::old::codegen::machine::MachineDataRelocation,
...
fn lower_module_instruction(
    ...
    for (index, operand) in instruction.operands.iter().enumerate() {
        ...
        if !operand.is_register()
            && !matches!(
                operand.role,
                crate::old::codegen::machine::OperandRole::None
            )
        {
...
fn lower_module_operand(
    ...
    match &operand.kind {
        MachineOperandKind::Register(crate::old::codegen::machine::MachineRegister::Physical(
            register,
        )) => Ok(MCOperand::Register(mc::PhysicalRegister::new(
            register.get(),
        ))),
        MachineOperandKind::Register(crate::old::codegen::machine::MachineRegister::Virtual(_)) => {
...
mod tests {
    ...
    use crate::old::target::x86::{X86Opcode, X86Register, X86RegisterClass, verify_machine};
    ...
    fn signature() -> MachineSignature {
        MachineSignature {
            ...
            parameters: Vec::new(),
            variadic: false,
            calling_convention: crate::old::codegen::machine::MachineCallingConvention::C,
        }
    }
        ...
        let cases = vec![
            ...
            (
                operand(MachineOperandKind::FrameIndex {
                    index: crate::old::codegen::machine::FrameIndex::new(2),
                    addend: 0,
                }),
                McLowerError::UnresolvedOperand {
                    operand: 0,
