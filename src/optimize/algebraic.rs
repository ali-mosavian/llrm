...
pub(crate) fn simplified(
    ...
    let body = _constants_outward(&_reassociated_recurrences(&body));
...
/// Constants moved outward past a single-use offset: `(x + c) * k` becomes
/// `x * k + c * k`, and `x + (y + c)` becomes `(x + y) + c`.
///
/// Every copy of `x` scaled or based alike then names one value, which value
/// numbering shares: `a[i]` and `b[i - 1]` become `a + 2i` and `(b + 2i) - 2`,
/// one `2i` and a displacement. A recurrence's update is left to
/// `_reassociated_recurrences`, which wants the phi at its root.
pub(crate) fn _constants_outward(body: &MirBody) -> MirBody {
    let mut read = Counter::new();
    for op in body.blocks.iter().flat_map(|block| &block.ops) {
        let mut values = _operands_read(op);
        values.extend(op.uses.iter().copied());
        for value in values {
    ...
    let observed = read.keys().copied().collect::<BTreeSet<_>>();
    let updates = body
        .blocks
        .iter()
        .flat_map(|block| &block.phis)
    ...
    let place = body
        ...
        .iter()
        .enumerate()
        .flat_map(|(b, block)| block.ops.iter().enumerate().flat_map(move |(o, op)| op.defines.iter().map(move |value| (*value, (b, o)))))
        .collect::<BTreeMap<_, _>>();
    // The single-use offset `middle` feeding `op`, as its source and amount.
    let offset_into = |op: &Op, middle: Held| {
        let &at = place.get(&middle.value)?;
        let previous = &body.blocks[at.0].ops[at.1];
        if times(&read, &middle.value) != 1 || previous.results != [Arg::Held(middle)] {
            return None;
        }
        let (source, amount) = _offset(previous, &observed)?;
        (op.results.len() == 1).then_some((at, source, amount))
    };
    let mut rewritten = BTreeMap::<(usize, usize), Op>::new();
    for (b, block) in body.blocks.iter().enumerate() {
        for (o, op) in block.ops.iter().enumerate() {
            let Some(result) = op.results.first().and_then(held_of) else {
                continue;
            };
            if rewritten.contains_key(&(b, o)) || updates.contains(&result.value) {
                continue;
            }
            let pair = if let Some((middle, factor)) = _scale(op, &observed, false) {
                offset_into(op, middle).map(|(at, source, amount)| {
                    let width = source.width;
                    // The scale keeps its own kind and name, so it numbers with
                    // any other scale of `source`.
                    let inner = Op { args: vec![Arg::Held(source), op.args[1].clone()], uses: vec![source.value], ..op.clone() };
                    (at, middle, inner, consts::masked(&(amount * factor), width))
                })
            } else if let Some((left, right)) = _summands(op, &observed) {
                [(left, right), (right, left)].into_iter().find_map(|(other, middle)| {
                    let (at, source, amount) = offset_into(op, middle)?;
                    let inner = Op {
                        args: vec![Arg::Held(other), Arg::Held(source)],
                        uses: unique([other.value, source.value]),
                        ..op.clone()
                    };
                    Some((at, middle, inner, consts::masked(&amount, source.width)))
                })
            } else {
                None
            };
            let Some((at, middle, inner, amount)) = pair else {
                continue;
            };
            if rewritten.contains_key(&at) {
                continue;
            }
            let inner = Op {
                results: vec![Arg::Held(middle)],
                defines: vec![middle.value],
                source_backed: false,
                raised: None,
                ..inner
            };
            let outer = Op {
                ...
                args: vec![Arg::Held(middle), Arg::Const(Const::new(amount, middle.width))],
            ...
            rewritten.insert(at, inner);
            rewritten.insert((b, o), outer);
        }
    }
    ...
    body.with_blocks(
        body.blocks
            ...
            .map(|(b, block)| {
                block.with_ops(
                    block.ops.iter().enumerate().map(|(o, op)| rewritten.get(&(b, o)).cloned().unwrap_or_else(|| op.clone())).collect(),
                )
            })
            .collect(),
    )
...
/// A pure add of two held values at its own width.
pub(crate) fn _summands(op: &Op, wanted: &BTreeSet<Value>) -> Option<(Held, Held)> {
    if op.kind != Kind::Add
        || !op.loads.is_empty()
        || !op.stores.is_empty()
        || op.barrier()
    ...
    let (Arg::Held(left), Arg::Held(right)) = (&op.args[0], &op.args[1]) else {
        return None;
    };
    (left.width == result.width && right.width == result.width).then_some((*left, *right))
}
