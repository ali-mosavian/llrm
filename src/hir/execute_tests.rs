...
fn loops_and_internal_calls_run() {
    let source = include_str!("../../fixtures/modern/control.mod");
...
fn print_captures_text_integers_floats_and_fixed() {
    ...
    let fixed = include_str!("../../fixtures/modern/fixed.mod");
...
fn nbody_matches_the_python_reference() {
    let executed = execute(
        include_str!("../../fixtures/modern/nbody.mod"),
        "nbody",
        &[Number::Int(1)],
    );
    assert!(
