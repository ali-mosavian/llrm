...
use crate::frontend::qb::dialect_extensions::{ExtensionAction, recognize_statement};
use crate::frontend::qb::error::ParseError;
use crate::frontend::qb::syntax::{
    ...
    let statement = match descriptor {
        ...
        StatementShape::ExitProcedure | StatementShape::ExitDo | StatementShape::ExitFor => {
            ...
            let target = match descriptor {
                StatementShape::ExitProcedure => crate::frontend::qb::syntax::ExitTarget::Sub,
                StatementShape::ExitDo => crate::frontend::qb::syntax::ExitTarget::Do,
                StatementShape::ExitFor => crate::frontend::qb::syntax::ExitTarget::For,
                _ => unreachable!("matched exit statement shape"),
            };
            let target = if target == crate::frontend::qb::syntax::ExitTarget::Sub
            ...
            {
                crate::frontend::qb::syntax::ExitTarget::Function
            } else {
                target
            };
        ...
        StatementShape::Open => {
            let mode = if actions
                ...
                .any(|action| matches!(action, AstAction::Mark { slot: 1, .. }))
            {
                crate::frontend::qb::syntax::FileMode::Append
            } else if actions
                .iter()
                .any(|action| matches!(action, AstAction::Mark { slot: 2, .. }))
            {
                crate::frontend::qb::syntax::FileMode::Input
            } else if actions
                .iter()
                .any(|action| matches!(action, AstAction::Mark { slot: 3, .. }))
            {
                crate::frontend::qb::syntax::FileMode::Output
            } else if actions
                .iter()
                .any(|action| matches!(action, AstAction::Mark { slot: 5, .. }))
            {
                crate::frontend::qb::syntax::FileMode::Binary
            } else {
                return false;
            };
        ...
        StatementShape::Resume => {
            let target = if actions
                ...
                .any(|action| matches!(action, AstAction::Mark { slot: 4, .. }))
            {
                crate::frontend::qb::syntax::ResumeTarget::Next
            } else if let [(label, _)] = labels.as_slice() {
                crate::frontend::qb::syntax::ResumeTarget::Label(label.clone())
            } else if literal_values.as_slice() == [0] {
                crate::frontend::qb::syntax::ResumeTarget::Label("0".into())
            } else if labels.is_empty() && literal_values.is_empty() {
                crate::frontend::qb::syntax::ResumeTarget::Current
            } else {
                return false;
            };
...
mod tests {
    use super::*;
    use crate::frontend::qb::syntax::{ExitTarget, FileMode, ResumeTarget};
