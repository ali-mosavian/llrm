...
use super::state::{BranchTarget, EncodeConfig, StateDecoder, StateEntry};
use super::tables;
...
mod tests {
    use super::*;
    use buildprs::buildprs_encoder::{
        BranchTarget as BuildprsBranchTarget, EncodeConfig as BuildprsEncodeConfig, StateEncoder,
    };
    ...
    fn rejected_alternative_restores_emitted_actions_and_ast_builder() {
        let config = BuildprsEncodeConfig::new(ENCODE1BYTE);
        ...
        let tokens = crate::frontend::qb::generated_parser::lex(
            "",
            crate::frontend::qb::dialect::Dialect::QuickBasic45,
        )
        .unwrap();
        let mut state = ParseState::new(tokens);
        state.expressions.push(Expr::Name(
            "SENTINEL".into(),
            crate::frontend::qb::syntax::Span {
                line: 1,
                start: 0,
                end: 0,
            },
