...
use crate::analysis::ranges::singletons;
...
pub const CELLS: [Space; 2] = [Space::Segment, Space::Frame];
...
pub fn _leaf(r#ref: &MemRef) -> Option<_Leaf> {
...
pub fn _blocked(refs: &[&MemRef]) -> BTreeSet<Slice> {
...
pub fn _key(r#ref: &MemRef, blocked: &BTreeSet<Slice>) -> Option<Key> {
    if r#ref.volatile {
        return None;
    }
...
pub fn _order(key: &Key) -> (u8, String, String, i64, i64, i64, String) {
...
pub fn _signed(number: &BigInt, width: u32) -> BigInt {
...
pub fn _affine_values(body: &MirBody) -> IndexMap<Value, _Affine> {
...
pub fn _allocation_leaves(body: &MirBody) -> MirBody {
...
pub fn _bounded_ref(r#ref: &MemRef, known: &IndexMap<Value, Interval>) -> MemRef {
...
pub fn _pointed_ref(r#ref: &MemRef, pointers: &IndexMap<Value, Provenance>) -> MemRef {
...
pub fn _bounded_leaves(body: &MirBody) -> Result<MirBody, String> {
...
pub fn _copy_partition(r#ref: &MemRef, leaves: &[_Leaf]) -> Option<Vec<_Leaf>> {
...
pub fn _copy_piece(r#ref: &MemRef, whole: &_Leaf, piece: &_Leaf, low: i64) -> MemRef {
...
pub fn _split_copies(body: &Rc<MirBody>) -> Rc<MirBody> {
    let leaves = body
        .blocks
        .iter()
...
pub fn _cell(op: &Op) -> Option<MemRef> {
...
pub fn _available(
    ...
    let layout = layout(bounds);
    let reachable = loops::dominators(&body.blocks, Some(body.entry))
        .into_iter()
...
pub fn _separated(body: &MirBody, objects: Option<&BTreeSet<MemoryObject>>) -> MirBody {
    let mut fresh = _next(body);
    let mut variable = ssa::values(body).map(|one| one.variable).max().unwrap_or(0) + 1;
    let mut blocks = Vec::new();
    for block in &body.blocks {
