...
impl Strength {
    pub fn new(r#where: crate::model::passes::Where) -> Self {
        Self { r#where }
    }
}
...
pub fn _starts(body: &MirBody, into: Value, one: &Derived, preheader: i64, counted: bool) -> Vec<Op> {
    let mut number = 0;
    let mut temporary = || {
        number += 1;
...
pub fn _start_temporary_count(one: &Derived, counted: bool) -> u32 {
    let unit = matches!(&one.by, Arg::Const(constant) if constant.n == BigInt::from(1_u8));
