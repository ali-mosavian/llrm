...
fn test_nbody_reuses_the_whole_signed_initialization_value() {
    let (_, _, body) = raised_main(concat!(env!("LLRM_ROOT"), "/tests/fixtures/bench/nbody-v-g3.obj"));
...
fn test_whole_store_requires_adjacent_matching_word_writes() {
    let (_, _, raised) = raised_main(concat!(env!("LLRM_ROOT"), "/tests/fixtures/bench/nbody-v-g3.obj"));
...
fn test_nbody_multiply_value_survives_into_scaled_division() {
    let done = transformed(concat!(env!("LLRM_ROOT"), "/tests/fixtures/regressions/nbody-stack-p-g2.obj"), Options::default());
...
fn test_nbody_address_shifts_combine_without_an_extra_counter() {
    let (_, _, body) = raised_main(concat!(env!("LLRM_ROOT"), "/tests/fixtures/regressions/nbody-stack-p-g2.obj"));
...
fn test_harr_only_needs_the_low_product() {
    for tag in ["p-g2", "q-o", "v-g3"] {
        let (_, _, before) = raised_main(&format!("{}/tests/fixtures/omf/harr-{tag}.obj", env!("LLRM_ROOT")));
...
fn test_dead_phis_do_not_keep_matrix_product_halves() {
    let after = transformed(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/matrix-p-g2.obj"), Options { strength: false, ..Default::default() });
    let products: Vec<&Op> = all_ops(&after).filter(|op| op.kind == Kind::Mul).collect();
...
fn test_nbody_counter_is_stored_as_one_whole_value() {
    let (found, blocks, body) = raised_main(concat!(env!("LLRM_ROOT"), "/tests/fixtures/bench/nbody-v-g3.obj"));
    let done = simplified(&body, &BTreeSet::new(), &BTreeSet::new()).unwrap();
