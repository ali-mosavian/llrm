...
/// Replaying each stage's record from the entry body's operations gives
/// exactly the final body's: nothing a pass or driver did goes unrecorded.
#[test]
fn test_the_stage_records_account_for_every_operation() {
    use std::collections::BTreeSet;

    use crate::model::mir::{Disposition, MirBody, identified};
