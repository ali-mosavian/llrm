...
pub fn entered(body: &Rc<MirBody>) -> Result<Rc<MirBody>, SubstitutionError> {
...
pub fn _counted_down(body: &Rc<MirBody>) -> Result<Rc<MirBody>, SubstitutionError> {
    let blocks = body
        .blocks
        .iter()
        .enumerate()
...
pub fn rotated(body: &Rc<MirBody>) -> Result<Rc<MirBody>, SubstitutionError> {
    let blocks = body
        .blocks
        .iter()
        .enumerate()
...
pub fn _step_test(body: &Rc<MirBody>, loop_: &Loop, header: &MirBlock) -> Rc<MirBody> {
    ...
    for counter in induction::basics(body, loop_).values() {
        ...
        return Rc::new(body.with_blocks(rewritten));
    }
    body.clone()
}
...
pub fn _swapped(op: &Op, swap: &BTreeMap<u32, Value>) -> Result<Op, SubstitutionError> {
