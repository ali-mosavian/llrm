...
fn test_nested_accumulator_is_stored_only_after_the_outer_loop() {
    let found = testing::module(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/nested-p-g2.obj"));
...
fn test_a_float_loop_sinks_its_counter_store_without_an_error_handler() {
    let (_, states) = testing::emitted_states(&testing::data(concat!(env!("LLRM_ROOT"), "/tests/fixtures/bench/nbodys-v-g3.obj")));
