...
pub fn _forwarded_zero_tests(body: &MirBody) -> Result<MirBody, String> {
    let definitions = definitions_of(body);
    let phi_inputs = body
        .blocks
        .iter()
...
pub fn _reassociated_recurrences(body: &MirBody) -> MirBody {
    let indexed = body
        .blocks
        .iter()
...
pub fn _shared_shifts(body: &MirBody, wanted: &BTreeSet<Value>) -> MirBody {
    let mut used = body
        .blocks
        .iter()
        .flat_map(|block| &block.ops)
...
pub fn _scale(op: &Op, wanted: &BTreeSet<Value>, tied: bool) -> Option<(Held, BigInt)> {
...
pub fn _offset(op: &Op, wanted: &BTreeSet<Value>) -> Option<(Held, BigInt)> {
...
pub fn _recombined<'a>(op: &'a Op, definitions: &Definitions<'_>) -> Cow<'a, Op> {
...
pub fn _extracted<'a>(op: &'a Op, definitions: &Definitions<'_>) -> Cow<'a, Op> {
...
pub fn _redundant_extension<'a>(op: &'a Op, definitions: &Definitions<'_>) -> Cow<'a, Op> {
...
pub fn _zero_difference<'a>(op: &'a Op, definitions: &Definitions<'_>) -> Cow<'a, Op> {
    if op.kind != Kind::Sub
        || !op.loads.is_empty()
        || !op.stores.is_empty()
        || op.barrier()
...
pub fn _copied_zero(arg: &Arg, definitions: &Definitions<'_>) -> bool {
...
pub fn _halves(op: &Op) -> Option<(Arg, Arg)> {
    if op.kind != Kind::Concat
        || !op.loads.is_empty()
        || !op.stores.is_empty()
        || op.barrier()
...
pub fn _takes_halves(op: &Op, whole: Held, readers: &BTreeMap<Value, Vec<&Op>>) -> bool {
...
pub fn _halved(body: &MirBody) -> MirBody {
    let joins = body
        .blocks
        .iter()
        .flat_map(|block| &block.ops)
...
pub fn _divisions(body: &Rc<MirBody>) -> Rc<MirBody> {
    if !body
        .blocks
        .iter()
        .flat_map(|block| &block.ops)
...
pub fn _product<'a>(op: &'a Op, wanted: &BTreeSet<Value>, wide: &BTreeSet<Value>) -> Cow<'a, Op> {
    if op.kind != Kind::Mul
        || op.barrier()
        || !op.stores.is_empty()
        || op.results.len() != 2
...
pub fn _operands_read(op: &Op) -> BTreeSet<Value> {
    op.args
        .iter()
        .filter_map(held_of)
        .map(|held| held.value)
...
pub fn _simplified<'a>(op: &'a Op, wanted: &BTreeSet<Value>, wide: &BTreeSet<Value>) -> Cow<'a, Op> {
