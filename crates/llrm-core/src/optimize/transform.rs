...
pub fn _without(ops: &[Op], drop: impl Fn(&Op) -> bool) -> Vec<Op> {
    let mut out = Vec::new();
    for op in ops {
        if !drop(op) {
            out.push(op.clone());
...
pub fn _phi_reading(phi: &Phi, swap: &BTreeMap<u32, Value>) -> Result<Phi, String> {
...
pub fn subexpressions(body: &Rc<MirBody>, dgroup: &BTreeSet<i64>, avoid_store_crossing: bool) -> Result<Rc<MirBody>, String> {
...
pub fn _unchanged_float_environment(op: &Op) -> bool {
    use crate::model::floating::Exceptions;
...
pub fn _exact_floating(op: &Op, facts: &IndexMap<Value, crate::analysis::floatfacts::Finite>) -> bool {
...
pub fn _erased_floating(op: &Op) -> Op {
    Op {
...
pub fn _reclaimed(body: &MirBody, gone: &BTreeSet<OpOccurrence>) -> MirBody {
...
pub fn _widths(body: &MirBody) -> IndexMap<u32, u32> {
...
pub fn _full(one: &Held, whole: &IndexMap<u32, u32>) -> bool {
    whole.get(&one.value.id) == Some(&one.width)
}

/// The value this operation is another name for, if it is only that.
pub fn _copied(op: &Op, whole: &IndexMap<u32, u32>) -> Option<Value> {
...
pub fn _width(_value: Value, op: &Op) -> Option<u32> {
...
pub fn _computation(op: &Op, stands: &IndexMap<u32, Value>, whole: &IndexMap<u32, u32>) -> Option<_Computation> {
...
pub fn _read(body: &MirBody, value: Value) -> bool {
...
pub fn divided_twice<'a>(body: &'a MirBody, dgroup: &BTreeSet<i64>) -> Vec<(i64, &'a Op, &'a Op)> {
...
pub fn _undisturbed(one: &Op, _earlier: &Op, between: &[Op], _dgroup: &BTreeSet<i64>) -> bool {
    let cells = one
        .args
        .iter()
        .filter_map(|arg| match arg {
...
pub fn placed(body: &Rc<MirBody>, dgroup: &BTreeSet<i64>, calls: &IndexMap<i64, String>) -> Result<Rc<MirBody>, String> {
...
pub fn _may_pass(one: &Op, run: &[Op], _dgroup: &BTreeSet<i64>, calls: &IndexMap<i64, String>) -> bool {
...
pub fn _meets(one: Option<&BTreeSet<String>>, other: Option<&BTreeSet<String>>) -> bool {
    match (one, other) {
...
pub fn _empty_operation(op: &crate::model::mir::Op) -> crate::model::mir::Op {
...
pub fn _served(op: &Op, holder: &crate::analysis::avail::Holder) -> Option<Vec<Arg>> {
...
pub fn _preheader(body: &MirBody, loop_: &Loop) -> Option<i64> {
    let outside = body
        .blocks
        .iter()
...
pub fn _effective(body: &MirBody, calls: &IndexMap<i64, String>) -> BTreeSet<Value> {
    let _ = calls;
...
pub fn _cannot_fault(op: &Op) -> bool {
...
pub fn _whole_shift(op: &Op, readable: Option<&BTreeSet<Value>>) -> bool {
...
pub fn _complete_value(op: &Op, readable: Option<&BTreeSet<Value>>) -> bool {
...
/// `mir.consumed`.
pub fn _consumed(op: &Op) -> BTreeSet<Value> {
    mir::consumed(op)
}
...
pub fn motion_blocked<'a>(ops: impl IntoIterator<Item = &'a Op>) -> bool {
...
pub fn _pruned<'a>(run: &[&'a Op], drop: &BTreeSet<Value>) -> Vec<&'a Op> {
    let mut keep = run
        .iter()
        .copied()
...
pub const _OBSERVED: [crate::model::mir::Kind; 20] = {
...
pub fn _leaving(body: &MirBody) -> std::collections::BTreeSet<crate::model::mir::Value> {
...
pub fn _reusing_halves<T>(inside: impl FnOnce() -> T) -> T {
...
pub fn halves(body: &Rc<MirBody>) -> BTreeSet<(Value, u8)> {
...
pub fn live(body: &Rc<MirBody>) -> std::collections::BTreeSet<crate::model::mir::Value> {
...
pub fn _signed(fact: &crate::analysis::consts::Known) -> num_bigint::BigInt {
...
pub fn _switch_target(op: &Op, facts: &IndexMap<Value, crate::analysis::consts::Known>) -> Option<i64> {
    use crate::analysis::consts;
...
pub fn _threaded(body: &Rc<MirBody>) -> Result<Rc<MirBody>, String> {
...
pub fn _unreachable(body: &MirBody) -> MirBody {
    use std::collections::{BTreeMap, BTreeSet};
...
pub fn _trivial_phis(body: &MirBody) -> Result<MirBody, String> {
    use std::collections::{BTreeMap, BTreeSet};
...
pub fn dead(body: &Rc<MirBody>) -> Result<Rc<MirBody>, String> {
...
pub fn _overwritten_locally(block: &MirBlock) -> BTreeSet<Value> {
...
pub fn _kept(op: &crate::model::mir::Op) -> bool {
    use crate::model::mir::Kind;
...
pub fn _removable(op: &Op, alive: &BTreeSet<Value>) -> bool {
    if _kept(op) {
        return false;
    }
...
pub fn _folded_division(op: &Op, numbers: (num_bigint::BigInt, num_bigint::BigInt), wanted: &BTreeSet<Value>) -> Vec<Op> {
    use crate::model::ir::Operation;
...
pub const _EDGE_FOLDABLE: [Kind; 25] = {
...
pub fn folded(body: &Rc<MirBody>, dgroup: &BTreeSet<i64>, calls: &IndexMap<i64, String>) -> Result<Rc<MirBody>, String> {
...
pub fn _constant_fill(op: &Op, facts: &IndexMap<Value, crate::analysis::consts::Known>) -> Op {
    use crate::analysis::consts;
...
pub fn _constant_based(op: Op, facts: &IndexMap<Value, crate::analysis::consts::Known>) -> Op {
...
pub fn _symbol_copies(body: &MirBody) -> _SymbolCopies<'_> {
    body.blocks
        .iter()
        .flat_map(|block| &block.ops)
        .filter(|op| {
...
pub fn _folded_op(op: &Op, facts: &IndexMap<Value, crate::analysis::consts::Known>, wanted: &BTreeSet<Value>) -> Op {
    use crate::model::mir::Const;
...
pub fn _reparented(body: &MirBody, crossed: &crate::support::pyset::PySet<Value>) -> MirBody {
...
pub fn _guaranteed_float_work(body: &MirBody, loop_: &Loop, nonempty: bool) -> BTreeSet<usize> {
    if !nonempty {
        return BTreeSet::new();
    }
...
pub fn _handles_errors(r#where: &crate::model::passes::Where) -> bool {
...
pub static PASSES: std::sync::LazyLock<Vec<String>> = std::sync::LazyLock::new(|| {
...
#[allow(dead_code)]
pub static PASSES_ON: std::sync::LazyLock<Vec<String>> =
