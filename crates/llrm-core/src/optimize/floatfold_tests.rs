...
fn test_original_wait_is_an_explicit_checkpoint_with_encoding_provenance() {
    let body = testing::nth(&testing::raised_with(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/fpcse-p-g2.obj"), true, false), 0);
...
fn test_qb_fpcse_preserves_entry_when_first_load_disappears() {
    testing::emitted_lir(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/fpcse-q-o.obj"));
}
...
fn test_collapsed_fpcse_has_no_empty_jump_trampoline() {
    assert!(!emitted(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/fpcse-q-o.obj")).iter().any(|one| one.mnemonic() == iced_x86::Mnemonic::Jmp));
}
