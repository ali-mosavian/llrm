...
pub fn named(body: &MirBody) -> Result<MirBody, Unlowered> {
...
pub fn _indirect_call(op: &Op) -> Result<bool, Unlowered> {
    if !op.indirect {
        return Ok(false);
    }
...
pub fn semantics(op: &Op, was: Option<&ir::Semantics>, place: Place) -> Result<Option<Unlocated>, Unlowered> {
...
pub fn as_a_value(arg: &Arg, _had: &[Loc], _index: usize) -> Placed {
    operand(arg)
}

/// `what` with every register the operation names as a value replaced by it.
pub fn _valueized(what: &ir::Semantics, op: &Op) -> ir::Semantics {
...
pub fn _place(arg: &Arg, had: &[Loc], index: usize) -> Placed {
    let got = operand(arg);
...
pub fn rewritten(op: &Op, place: Place, node: Option<&Node>) -> Result<Option<ir::Semantics>, Unlowered> {
    let was = node.map(Node::semantics);
    _located(semantics(op, was, place)?, was)
}

/// What this operation computes now, in machine form.
pub fn current(op: &Op, place: Place, node: Option<&Node>) -> Result<Option<ir::Semantics>, Unlowered> {
...
pub fn _selector(reference: &MemRef) -> Option<ir::Held> {
...
pub fn _addressed(one: &MemRef) -> Result<ir::Mem, Unlowered> {
...
pub fn _check_inserted_conditions(
    ops: &[Op],
...
impl<'a> Lowering<'a> {
    ...
    pub fn _idiom(&self, op: &Op, speaks: bool) -> Result<Vec<(ir::Held, Register)>, Unlowered> {
