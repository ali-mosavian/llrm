...
pub fn local(body: &LirBody, value: u32, index: &Indexes, live: (&allocate::Live, &allocate::Live), candidates: &[Register], occupied: &Occupied, width: u32) -> Option<Region> {
    ...
    let named: Vec<usize> = (0..block.insns.len())
        .filter(|at| block.insns[*at].defines.contains(&value) || block.insns[*at].uses.contains(&value))
        .collect();
    let slot = |position: usize| index.slot(block, position);
...
mod tests {
    ...
    /// A meta instruction takes no slot, but a piece's slots were counted one
    /// per position, so a piece after a byte marker reached past its code.
    #[test]
    fn test_a_piece_after_a_meta_instruction_starts_at_its_own_slot() {
        let marker = Arc::new(Insn::new(2, Some((2, 3)), Some(crate::model::lir::inert()), Vec::new(), Vec::new()));
        let insns = vec![move_imm(0, 3, 0x40), marker, move_imm(4, 4, 1), push(6, 3), push(8, 4)];
        let body = body("marked", vec![block(0, insns, &[])]);
