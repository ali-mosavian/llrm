...
pub fn _span(fact: Option<&Fact>, width: u32) -> Option<Interval> {
    match fact? {
...
pub fn _fitted(fact: Option<&Fact>, width: u32) -> Option<Fact> {
    match fact? {
...
pub fn _joined(facts: &[Option<Fact>], width: u32) -> Option<Fact> {
...
pub fn _joined_values(facts: &[Option<(Option<Fact>, u32)>]) -> Option<(Option<Fact>, u32)> {
    if facts.is_empty() || facts.iter().any(Option::is_none) {
        return None;
    }
...
pub fn _meet(states: &[State]) -> State {
...
pub fn _address(reference: &MemRef, state: &State) -> Option<Address> {
...
pub fn _read(arg: &Arg, state: &State) -> Option<Fact> {
    match arg {
...
pub fn _result(op: &Op, args: &[Option<Fact>]) -> Option<Fact> {
...
pub fn _overlap(left: &Address, width: u32, right: &Address, size: u32) -> bool {
    match (left, right) {
...
pub fn _transfer(block: &MirBlock, arriving: &State, cyclic: bool) -> (State, IndexMap<(usize, MemRef), Checked>) {
...
pub fn _edge(state: &State, block: &MirBlock, successor: i64) -> Option<State> {
    let mut known = IndexMap::default();
...
pub fn _widened(previous: Option<&Fact>, current: Option<&Fact>, width: u32) -> Option<Fact> {
...
pub fn _widen(previous: &State, current: &mut State) {
