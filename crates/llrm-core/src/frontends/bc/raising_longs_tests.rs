...
fn test_udtacc_second_field_is_a_whole_long() {
    let body = nth(&testing::raised(concat!(env!("LLRM_ROOT"), "/tests/fixtures/regressions/udtacc-p-g2.obj")), 0);
...
fn test_nbody_timing_helper_stores_the_signed_whole_value() {
    let body = nth(&testing::raised(concat!(env!("LLRM_ROOT"), "/tests/fixtures/bench/nbody-v-g3.obj")), 1);
    let stores: Vec<Op> =
...
fn test_nbody_counter_seed_has_a_known_high_word() {
    ...
    let path = concat!(env!("LLRM_ROOT"), "/tests/fixtures/bench/nbody-v-g3.obj");
...
fn test_control_branch_updates_are_whole_longs_before_optimization() {
    let raised = testing::raised(concat!(env!("LLRM_ROOT"), "/tests/fixtures/parity/control-v-g3.obj"));
...
fn test_event_arithmetic_keeps_a_pair_when_its_flags_cross_the_machine_exit() {
    testing::emitted_lir(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/arith-p-evt.obj"));
}
...
fn test_nbody_whole_position_loads_leave_the_inner_loop() {
    let found = testing::module(concat!(env!("LLRM_ROOT"), "/tests/fixtures/regressions/nbody-stack-p-g2.obj"));
    let blocks = testing::blocks_of(&found);
    let body = testing::main_body(&found, &blocks);
...
fn test_removed_half_flags_do_not_become_machine_exit_inputs() {
    testing::emitted_lir(concat!(env!("LLRM_ROOT"), "/tests/fixtures/parity/control-v-g3.obj"));
}
...
fn test_production_does_not_recognize_long_pairs_after_optimization() {
    testing::emitted_lir(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/arith-p-g2.obj"));
}
