...
use crate::frontends::bc::raising_literals;
...
fn _copy(byte: Option<u8>) -> (Module, RaisedBody) {
    let path = concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/fpdeep-p-g2.obj");
...
fn test_proven_copy_unlocks_strict_floating_cse() {
    let (found, body) = _copy(Some(0xfc));
    let original = testing::nth(&testing::raised(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/fpdeep-p-g2.obj")), 0);
