...
fn test_overflow_observation_has_no_normal_path_register_results() {
    let path = concat!(env!("LLRM_ROOT"), "/tests/fixtures/regressions/arridx-bounds-p-g2.obj");
    let found = testing::loaded(path).unwrap();
...
fn test_selector_proof_checks_the_loop_exit_path() {
    let path = concat!(env!("LLRM_ROOT"), "/tests/fixtures/regressions/hugelp-p-g2.obj");
    let found = testing::loaded(path).unwrap();
...
fn test_bounds_policy_is_recorded_separately() {
    let source = concat!(env!("LLRM_ROOT"), "/tests/fixtures/regressions/arridx-bounds-p-g2.obj");
