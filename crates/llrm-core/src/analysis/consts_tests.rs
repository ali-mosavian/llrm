...
fn test_relocated_descriptor_address_is_not_integer_zero() {
    let obj = concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/harr-p-g2.obj");
    let found = testing::loaded(obj).unwrap();
...
fn test_folded_extraction_has_no_implicit_machine_result() {
    let found = testing::module(concat!(env!("LLRM_ROOT"), "/tests/fixtures/regressions/chain-stack-q-o.obj"));
...
fn test_a_comparison_result_folds_to_basics_own_true() {
    let mut seen = 0;
    for body in raised(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/cmpord-p-evt.obj")) {
        let facts = known(&body, None, None, None, None);
