...
#[derive(Clone)]
pub struct CellMap<K, V, B: Bucket> {
    items: IndexMap<K, V>,
...
thread_local! {
    ...
    pub static VERIFYING: std::cell::Cell<bool> = const { std::cell::Cell::new(false) };
    pub static VERIFIED: std::cell::Cell<usize> = const { std::cell::Cell::new(0) };
}
