...
pub static ARITH: [(Kind, Binary); 8] = [
    (Kind::Add, |a, b| a + b),
    (Kind::Sub, |a, b| a - b),
    (Kind::And, |a, b| a & b),
    (Kind::Or, |a, b| a | b),
...
pub static UNARY: [(Kind, Unary); 2] = [(Kind::Neg, |a| -a), (Kind::Not, |a| !a)];
...
pub type HeldCells = IndexMap<(i64, usize), Rc<Cells>>;
...
impl _MemoryQueries {
    pub fn new(known: &IndexMap<Value, Known>, dgroup: &BTreeSet<i64>) -> Self {
    ...
    pub fn learn(&mut self, reference: &MemRef) {
...
pub fn updated(op: &Op, known: &IndexMap<Value, Known>, here: &Cells) -> Option<Known> {
    if op.barrier()
        || op.floating.is_some()
        || !op.merges.is_empty()
        || op.stores.len() != 1
...
pub fn _defined(op: &Op) -> Option<Value> {
...
pub fn _carry(op: &Op, facts: &IndexMap<Value, Known>, here: &Cells) -> Option<BigInt> {
