...
impl PhiOccurrence {
    ...
    pub const fn phi_index(self) -> usize {
        self.phi_index
    }
}

/// Enumerate every operation in Python body/block/operation order.
pub fn operations(body: &MirBody) -> impl Iterator<Item = (OpOccurrence, &MirBlock, &Op)> {
    body.blocks
        .iter()
        .enumerate()
...
pub fn phis(body: &MirBody) -> impl Iterator<Item = (PhiOccurrence, &MirBlock, &Phi)> {
    body.blocks
        .iter()
        .enumerate()
        .flat_map(|(block_index, block)| {
