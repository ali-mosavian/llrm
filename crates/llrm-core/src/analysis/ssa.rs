...
pub fn substituted(op: &Op, swap: &BTreeMap<u32, Value>) -> Result<Op, SubstitutionError> {
    if swap.is_empty() {
        return Ok(op.clone());
    }
...
pub fn cloned_pointer_metadata<'a, M: IntoIterator<Item = (&'a u32, &'a Value)>>(
    body: &MirBody,
...
pub fn cloned_integer_ranges<'a, M: IntoIterator<Item = (&'a u32, &'a Value)>>(
    body: &MirBody,
    mappings: impl Iterator<Item = M>,
...
pub fn renumbered(body: &MirBody, variable: u32) -> MirBody {
