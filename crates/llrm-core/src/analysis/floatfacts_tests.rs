...
fn test_a_loop_exit_repeats_its_stores_every_iteration() {
    ...
    let data = std::fs::read(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/fpcse-p-g2.obj")).unwrap();
...
fn test_entry_bytes_are_killed_by_a_store() {
    let found = testing::module(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/fpcse-p-g2.obj"));
    let body = testing::main_body(&found, &testing::blocks_of(&found));
