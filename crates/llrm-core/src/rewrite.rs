...
mod tests {
    ...
    const HOTLOP: &str = concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/hotlop-q-evt.obj");
    const FIXTURE: &str = concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/lngmxx-p-g2.obj");
    ...
    fn test_an_escaping_exception_is_named_as_python_names_it() {
        let raised = once(&std::fs::read(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/jumps-p-evt.obj")).unwrap(), true).unwrap_err();
    ...
    fn test_stale_dependency_does_not_overwrite_cli_output() {
        ...
        let source = std::fs::read(concat!(env!("LLRM_ROOT"), "/tests/fixtures/regressions/qrender-main-v-g3.obj")).unwrap();
        let dependency = std::fs::read(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/hotlop-p-g2.obj")).unwrap();
        ...
        let argv: Vec<String> = [concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/hotlop-p-g2.obj"), "--contracts", path.to_str().unwrap(), "-o", output.to_str().unwrap()]
            .into_iter()
            .map(str::to_owned)
            .collect();
