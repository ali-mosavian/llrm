...
fn fixtures() -> Vec<PathBuf> {
    let mut found: Vec<PathBuf> = std::fs::read_dir(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf"))
        .unwrap()
...
fn test_absorbed_division_survives_removed_call_result_pins() {
    for name in ["qb45", "pds-g2"] {
        let data = std::fs::read(format!("{}/tests/fixtures/omf/{name}.obj", env!("LLRM_ROOT"))).unwrap();
...
fn test_a_loop_label_survives_coalescing_its_first_copy() {
    for name in ["hotlop", "press", "matrix", "jumps"] {
        let data = std::fs::read(format!("{}/tests/fixtures/omf/{name}-p-g2.obj", env!("LLRM_ROOT"))).unwrap();
        let (output, _) = rewrite(&data, &Rewrite::new(false)).unwrap();
        assert!(finalised(&output), "{name}");
    }
}
...
fn test_split_edges_do_not_create_phantom_padding() {
    for tag in ["p-g2", "v-g3"] {
        let result = default_emitted(&std::fs::read(format!("{}/tests/fixtures/omf/bools-{tag}.obj", env!("LLRM_ROOT"))).unwrap());
...
fn test_a_rewritten_operation_still_has_machine_operands() {
    for name in ["ivchan-q-o", "stride-p-g2"] {
        let raw = std::fs::read(format!("{}/tests/fixtures/omf/{name}.obj", env!("LLRM_ROOT"))).unwrap();
...
fn test_an_emission_says_which_emitter_produced_it() {
    let got = default_emitted(&std::fs::read(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/hotlop-p-g2.obj")).unwrap());
...
fn test_the_long_divide_bodys_entry_reads_nothing_it_has_not_written() {
    let got = default_emitted(&std::fs::read(concat!(env!("LLRM_ROOT"), "/tests/fixtures/omf/lngmix-p-g2.obj")).unwrap());
...
fn test_nbody_port_read_does_not_copy_neighbor_instructions() {
    let result = default_emitted(&std::fs::read(concat!(env!("LLRM_ROOT"), "/tests/fixtures/bench/nbody-v-g3.obj")).unwrap());
    assert_eq!(result.outcome, Emission::Lir, "{}", result.reason);
