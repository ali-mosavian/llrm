...
//! host (crates/llrm-nib/src/runtime/buffers.nib and strings.nib). A dropped buffer is marked, so a
...
impl Machine<'_> {
    ...
    /// crates/llrm-nib/src/runtime/dicts.nib's `N$DRES`: room for one more entry.
    ...
    /// code negated, as crates/llrm-nib/src/runtime/dos.asm returns them.
