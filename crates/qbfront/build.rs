...
fn main() -> io::Result<()> {
    ...
    let grammar = root.join("grammar/qbasbnf.prs");
    let peropcod = root.join("grammar/peropcod.txt");
    let action_schema = root.join("grammar/ast-actions.toml");
    let extension_schema = root.join("grammar/dialect-extensions.toml");
