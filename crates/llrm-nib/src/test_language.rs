...
";
    ...
    let checks = |hir: &str| hir.matches(&format!("\"callee\":\"{}\"", llrm_core::abi::nib::ERROR_BOUNDS)).count();
...
fn a_view_result_borrows_what_the_caller_lent_and_never_a_local() {
    let source = std::fs::read_to_string(concat!(env!("LLRM_ROOT"), "/examples/csv.nib")).expect("the example");
    assert_eq!(
        output_without_leaks(&source),
...
fn sequence_patterns_match_lengths_and_view_the_rest() {
    let source = std::fs::read_to_string(concat!(env!("LLRM_ROOT"), "/examples/sequences.nib")).expect("the example");
    assert_eq!(
        output_without_leaks(&source),
        "empty\none 7\n1..4 around 2\n4 -1\nfirst at 1,2\nstarts 1, 2 then 2\ntrue false\n"
    );
...
fn enumerate_zip_and_range_are_library_generators_yielding_references() {
    let source = std::fs::read_to_string(concat!(env!("LLRM_ROOT"), "/examples/laps.nib")).expect("the example");
    assert_eq!(
        output_without_leaks(&source),
        "lap 1: 62s\nlap 2: 58s\nlap 3: 60s\nlap 4: 57s\nfastest lap 4\nada made 10\nbob is 1 short\n\
...
fn fields_elements_and_views_are_places_like_names() {
    ...
    let source = std::fs::read_to_string(concat!(env!("LLRM_ROOT"), "/examples/gradebook.nib")).expect("the example");
    assert_eq!(
        output_without_leaks(&source),
...
";
    ...
    let source = std::fs::read_to_string(concat!(env!("LLRM_ROOT"), "/examples/borrows.nib")).expect("the example");
    assert_eq!(
        output_without_leaks(&source),
        "leader bob\nbest di with 58\nfirst cy\n[move]\n[north]\n[then]\n[east]\n"
    );
...
fn dicts_hash_their_keys_grow_and_lend_a_looked_up_key() {
    ...
    let source = std::fs::read_to_string(concat!(env!("LLRM_ROOT"), "/examples/tally.nib")).expect("the example");
    assert_eq!(
        output_without_leaks(&source),
...
fn a_main_returning_result_exits_0_on_ok_and_1_on_err() {
    ...
    assert_eq!(exit(".ok()"), ("hi\n".into(), Some(llrm_core::hir::model::Number::Int(0))));
    assert_eq!(exit(".err(.bad(3))"), ("hi\n".into(), Some(llrm_core::hir::model::Number::Int(1))));
}
...
fn the_roster_example_prints_compares_and_builds_players() {
    let source = std::fs::read_to_string(concat!(env!("LLRM_ROOT"), "/examples/roster.nib")).expect("the example");
    assert_eq!(
        output_without_leaks(&source),
...
fn the_easing_example_slides_through_a_table_of_function_values() {
    let source = std::fs::read_to_string(concat!(env!("LLRM_ROOT"), "/examples/easing.nib")).expect("the example");
    assert_eq!(
        output_without_leaks(&source),
...
fn the_readings_example_smooths_peeks_and_reports_generators() {
    let source = std::fs::read_to_string(concat!(env!("LLRM_ROOT"), "/examples/readings.nib")).expect("the example");
    assert_eq!(
        output_without_leaks(&source),
...
fn the_planets_example_prints_floats_as_the_shortest_text_that_reads_back() {
    let source = std::fs::read_to_string(concat!(env!("LLRM_ROOT"), "/examples/planets.nib")).expect("the example");
    assert_eq!(
        output_without_leaks(&source),
...
fn the_pipeline_example_streams_words_through_escaping_stages() {
    let source = std::fs::read_to_string(concat!(env!("LLRM_ROOT"), "/examples/pipeline.nib")).expect("the example");
    assert_eq!(
        output_without_leaks(&source),
        "skipped x\nmean 12\nmean 18\nmean 23\nskipped err\nmean 28\nmean 21\nmean 15\nclosed feed\nskipped x\nalarm at 23\nclosed alarm\ndone\n"
    );
