...
# llrm-nib compiles the program and src/frontends/nib/runtime/runtime.nib, jwasm
...
for part in start dos; do
    "$toolchain/jwasm" -q -c -Cp -Zg -omf "-Fo$work/$part.obj" "$root/src/frontends/nib/runtime/$part.asm"
...
"$bin/llrm-nib" "$root/src/frontends/nib/runtime/runtime.nib" -o "$work/runtime.obj" "$level" --procedure-segments $used >/dev/null
