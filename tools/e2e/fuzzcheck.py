...
BC's own build first and only then through llrm's rewritten one, across
...
place llrm's rewritten build disagrees with it, because those are different
bugs (evaluator or BC, versus llrm) and byte-diff against BC cannot tell them
...
passes per configuration:
  ...
  1. An identity transform, so `judge()`'s own BASEDIFF verdict means exactly
     "BC disagrees with the evaluator", with llrm not asked yet. This is
     ...
     before ever judging llrm against either.
  2. llrm's real rewrite, run only over the programs that passed pass 1 --
     a program BC itself could not build cleanly is not a llrm finding.
...
apparently, perfectly. Since llrm only ever sees objects BC successfully
...
untouched by anything llrm does -- is a cache hit on every re-run during
...
a change to llrm's own code changes those bytes, which changes the key, so
...
class TagReport:
    ...
    llrm_diffs: list[e2e.Verdict]  # llrm's rewrite disagrees with the evaluator
    passed: int

    @property
    def ok(self) -> bool:
        return not self.base_diffs and not self.llrm_diffs
...
def run_tag(
    ...
    opt = e2e.run(
        ...
        transform=None
        if absorb_calls
        else e2e.driver(e2e.rewriter_command(), CONFIGS[tag], "--no-absorb-calls"),
    )
    llrm_diffs = [v for v in opt.verdicts if not v.ok]
    passed = sum(1 for v in opt.verdicts if v.ok)
    return TagReport(tag, len(names), rejected + unjudged, base_diffs, llrm_diffs, passed)
...
def main(argv: list[str] | None = None) -> int:
    ...
    for r in reports:
        ...
        for v in r.llrm_diffs:
            print(f"    {v.status} (llrm regression) {v.program}: {v.detail}")
        if not r.ok:
            bad += 1
