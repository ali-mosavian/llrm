...
def linked_code_totals(bc: Path, qbopt: Path) -> tuple[int, int]:
    """Return whole-image executable code, including pulled runtime helpers."""
    return (
        segment_class_total(bc, LINKED_CODE_CLASSES),
        segment_class_total(qbopt, LINKED_CODE_CLASSES),
    )


def compare_maps(bc: Path, qbopt: Path) -> tuple[list[tuple[str, int, int]], tuple[int, int]]:
    baseline = code_segments(bc)
    candidate = code_segments(qbopt)
...
def markdown(bc: Path, qbopt: Path) -> str:
    rows, (bc_total, qbopt_total) = compare_maps(bc, qbopt)
    bc_linked, qbopt_linked = linked_code_totals(bc, qbopt)
    lines = [
        "| Module | BC bytes | qbopt bytes | Delta |",
        "| --- | ---: | ---: | ---: |",
    ]
    for name, bc_size, qbopt_size in rows:
        lines.append(f"| {name} | {bc_size:,} | {qbopt_size:,} | {signed(qbopt_size - bc_size)} |")
    lines.extend(
        [
            "| **BASIC-owned BC_CODE** | "
            f"**{bc_total:,}** | **{qbopt_total:,}** | **{signed(qbopt_total - bc_total)}** |",
            "| **Complete linked code** | "
            f"**{bc_linked:,}** | **{qbopt_linked:,}** | **{signed(qbopt_linked - bc_linked)}** |",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    parser.add_argument("qbopt", type=Path, help="MAP file from qbopt frontend output")
    args = parser.parse_args()
    try:
        report = markdown(args.bc, args.qbopt)
