...
def test_nbody_timer_can_be_emitted_without_fallback():
    ...
    assert any(fixup.target == "external" and fixup.loc == omf.LOC_OFF16
...
def test_external_symbols_are_not_proven_disjoint_by_their_indices():
    ...
    assert module.may_alias(first, second, frozenset(), a_width=2, b_width=2)
    assert module.may_alias(first, local, frozenset(), a_width=2, b_width=2)
