...
def test_nbody_repeated_fixed_constant_is_removed():
    """Nbody materialized 512 twice consecutively before the same fixed-input divide."""
    ...
    assert sum(one.insn.code == Code.MOV_R32_IMM32 and one.insn.immediate32 == 512
               for one in blocks.instructions(found)) == 2
...
@pytest.mark.parametrize("interruption", ["none", "call", "clobber", "unknown", "relocation", "block"])
def test_constant_knowledge_is_local_and_invalidated(interruption):
    ...
    assert sum(len(block.insns) for block in result.blocks) == (2 if interruption == "none" else 3)
