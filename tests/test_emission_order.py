...
def test_linear_placement_requires_one_complete_acyclic_chain(successors, expected):
    blocks = tuple(
        mir.MirBlock(at, (), (mir.Op(at, ir.Operation.NOTHING, "", (), ()),), succ)
...
def test_cloned_far_call_keeps_its_relocation_without_claiming_input_bytes():
    ...
    original = mir.Op(0, ir.Operation.CALL, "call", (), (), made=what, covers=(0, 5), id=7, symbol=True)
...
def test_block_entry_is_not_the_first_clone_of_its_source_address():
    ...
    header = move(3, 2, (3, 6))
    body = mir.MirBody(0, (mir.MirBlock(0, (), (initial, cloned_header), (3,)), mir.MirBlock(3, (), (header,), ())))
    found = SimpleNamespace(
        code=bytes.fromhex("b80100b80200"),
        end=6,
        coverage={},
