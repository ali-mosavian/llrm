...
def test_fpdeep_one_based_index_has_a_bounded_byte_offset(tag):
    ...
    accesses = [
        ...
        for op in block.ops
        for ref in op.loads
        if op.kind is mir.Kind.FLOAD and ref.base is not None
    ]
    assert accesses
...
def test_addrm_long_array_value_keeps_counter_bounds(tag):
    ...
    known = ranges.bounded(body)
    converted = [
        (block.at, op.results[0].value) for block in body.blocks for op in block.ops if op.kind is mir.Kind.SIGN_EXTEND
    ]
    assert converted
...
def test_nbody_scaled_index_is_bounded_only_inside_its_loop(recurrences, monkeypatch) -> None:
    ...
    if recurrences:
        ...
        monkeypatch.setattr(
            strength, "_multiplies", lambda one, derived: one.op.kind is mir.Kind.SHL or original(one, derived)
        )
...
@pytest.mark.parametrize(
    ("interval", "offset", "width", "disjoint"),
    [
