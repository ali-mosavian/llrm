...
def test_single_store_is_a_rounding_boundary(tag: str, basic_semantics: bool, exceptions: str) -> None:
    ...
    assert all(op.floating.exceptions == exceptions for op in (load, multiply, store))
...
def test_conversion_formats_do_not_confuse_integer_and_real(name, width, expected):
    ...
    op = mir.Op(
        0,
        ir.Operation.FLOAT_STORE if store else ir.Operation.FLOAT_LOAD,
        name,
        (),
        (),
...
@pytest.mark.parametrize(
    "name,precision,rounding", [("fchs", "exact", "none"), ("fabs", "exact", "none"), ("fsqrt", "dynamic", "dynamic")]
)
...
def test_dump_exposes_single_rounding(capsys):
    ...
    assert (
        "extended80 -> binary32; precision=destination rounding=dynamic exceptions=deferred" in capsys.readouterr().out
    )
