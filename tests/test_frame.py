...
def test_entry_frame_size_comes_from_cx_not_call_arity(registers):
    ...
    call = lir.Insn(
        ...
        defines=(),
        uses=tuple(value.value for value in operands),
    ...
    address = replace(
        init,
        at=8,
        what=ir.Semantics(ir.Operation.MOVE, "lea", (ir.Held(101, 2),), (ir.Address(ir.Addr(ir.Space.FRAME, -32), 0),)),
    )
    ...
    for required in ((), ((ir.Held(999, 2), Register.CX),)):
        invalid = replace(body, blocks=(replace(body.blocks[0], insns=(init, replace(call, requires=required))),))
