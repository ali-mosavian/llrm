...
def test_fpdeep_mix_outputs_fold_across_string_prints(tag):
    ...
    printed = {
        ...
        for arg in op.args
        if isinstance(arg, mir.Const) and arg.width == 4
    }
    assert {512, 768, 896} <= printed
    for body in states:
        literal = next(ref for ref, value in body.initial if value.n == 0x44800000 and value.width == 4)
        assert not any(
            op.kind is mir.Kind.FMUL and any(ref.addr == literal.addr for ref in op.loads)
            for block in body.blocks
            for op in block.ops
        )
    ...
    built = replace(
        ...
        blocks=tuple(
            replace(
                block,
                ops=tuple(
                    replace(op, stores=tuple(replace(ref, excludes=()) for ref in op.stores)) for op in block.ops
                ),
            )
...
@pytest.mark.parametrize(
    "offset,width,disjoint", [(0, 1, True), (3, 1, True), (0, 4, True), (-1, 2, False), (3, 2, False)]
)
...
def test_literal_bytes_are_available_to_scalar_loads(width):
    ...
    _, segment, start, payload = next(
        item
        for item in omf.ledata(found.records)
        if segments[item[1]][0] == "BC_CN" and item[2] <= 0x22 and item[2] + len(item[3]) >= 0x2A
    )
    ...
    assert consts.known(body, found.dgroup, {})[value] == consts.Known(int.from_bytes(payload[:width], "little"), width)


@pytest.mark.parametrize(
    "offset,location,admitted",
    [
...
def test_literal_relocation_exclusion_covers_the_whole_patch(monkeypatch, offset, location, admitted):
    ...
    path = Path("fixtures/omf/fpdeep-p-g2.obj")
    found = corpus.loaded(path)
    segment = next(index for index, item in enumerate(omf.segments(found.records)) if item and item[0] == "BC_CN")
...
def test_unknown_write_invalidates_literal_entry_facts():
    ...
    clobber = replace(
        ...
        args=(mir.Const(0, 4),),
        results=(mir.Cell(mir.MemRef(None, 4)),),
    ...
    body = replace(
        body,
        blocks=tuple(replace(block, ops=(clobber, *block.ops)) if block is entry else block for block in body.blocks),
    )
...
def test_literal_entry_requires_unmodified_complete_loader_bytes(change):
    ...
    record, index, start, payload = next(
        item for item in omf.ledata(found.records) if item[1] == ref.addr.index and item[2] == ref.addr.disp
    )
