use llrm::frontend::qb::semantic::{compile_with_array_order, compile_with_options};
...
fn resume_and_on_local_error_are_structured_control_transfers() {
    ...
    assert!(matches!(
        &module.statements[4],
        Statement::Resume {
            target: llrm::frontend::qb::syntax::ResumeTarget::Next,
            ..
        }
    ));
    assert!(matches!(
        &module.statements[5],
        Statement::Resume { target: llrm::frontend::qb::syntax::ResumeTarget::Label(label), .. }
            if label == "100"
    ));
    assert!(matches!(
        &module.statements[6],
        Statement::Resume {
            target: llrm::frontend::qb::syntax::ResumeTarget::Current,
            ..
        }
    ));
}
...
fn qb_intrinsics_resolve_through_one_declarative_catalogue() {
    use llrm::frontend::qb::intrinsics::{Effect, Lowering, ResultClass, find};
