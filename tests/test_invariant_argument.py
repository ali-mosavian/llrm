...
def test_argument_slot_is_loaded_once_before_the_emitted_loop(tag):
    ...
    slots = [(block.at, one) for block in decoded for one in block.insns
...
def test_loop_carried_argument_slot_load_moves_only_when_loop_executes(nonempty):
    ...
    body = transform.applied(body, found.dgroup, found.calls, found=found, hoist=False)
    loop, = loops.loops(body.blocks, body.entry)
    if not nonempty:
        header = body.block(loop.header)
        comparison = header.ops[-2]
        header = replace(header, ops=(*header.ops[:-2], replace(comparison,
            args=(comparison.args[0], mir.Const(0, 2))), header.ops[-1]))
        body = replace(body, blocks=tuple(header if block.at == header.at else block for block in body.blocks))
    def slot(op):
        return any(ref.addr is not None and ref.addr.space is module.Space.FRAME and ref.addr.disp == 6
                   for ref in op.loads)
    ...
    assert any(ref.base is not None for block in after.blocks if block.at in loop.body
