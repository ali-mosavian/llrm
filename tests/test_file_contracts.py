...
@pytest.mark.parametrize(
    "procedure,at,cleanup",
    [
        ("SYS_PARSE_ARGS", 0x12F, 2),
        ("SYS_PARSE_ARGS", 0x823, None),
        ("SYS_INIT_TABLES", 0x8F5, 0),
        ("SYS_ERROR", 0x980, None),
        ("SYS_ERROR", 0x989, 4),
        ("SYS_ERROR", 0x9A2, 4),
        ("SYS_TIME_INIT", 0x9CA, 0),
        ("SYS_MEM_MARK", 0xDC3, 2),
        ("SYS_MEM_MARK", 0xE22, 4),
    ],
)
...
def test_qbdemo_main_calls_lower(symbol):
    """qbdemo's main body was emitted as BC's bytes: these six had no interface.

    One refusal keeps a whole body, so every B$FIL2 raised to fild there and
    every DEF SEG store rewritten was thrown away with it.
    """
